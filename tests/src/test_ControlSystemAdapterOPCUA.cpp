...
void CSAOPCUATest::testWithoutPVSet(){ 
	...
	TestFixtureEmptySet tfEmptySet;
	 // Create the managers
	ControlSystemAdapterOPCUA *csaOPCUA = new ControlSystemAdapterOPCUA(tfEmptySet.csManager, tfEmptySet.syncCsUtility, "../tests/uamapping_test_1.xml");
	// is Server running?
	csaOPCUA->start();
...
void CSAOPCUATest::testWithPVSet(){ 
	...
	TestFixturePVSet tfExampleSet;
	 // Create the managers
	ControlSystemAdapterOPCUA *csaOPCUA = new ControlSystemAdapterOPCUA(tfExampleSet.csManager, tfExampleSet.syncCsUtility, "../tests/uamapping_test_2.xml");
	// is Server running?
	BOOST_CHECK(csaOPCUA->isRunning() == true);
