...
void ProcessVariableTest::testClassSide() {
    ...
    for (ProcessVariable::SharedPtr oneProcessVariable : pvSet.csManager->getAllProcessVariables()) {
        ua_processvariable *test = new ua_processvariable(serverSet->mappedServer, serverSet->baseNodeId,
                                                          oneProcessVariable->getName(), pvSet.csManager);
        ...
        if (valueType == "int8_t") {
            ...
            vector<int8_t> valueArray = pvSet.csManager->getProcessArray<int8_t>(
            ...
            if (valueArray.size() == 1) {
                ...
                vector<int8_t> csValueArray = pvSet.csManager->getProcessArray<int8_t>(
                        oneProcessVariable->getName())->accessChannel(0);
            ...
            vector<uint8_t> valueArray = pvSet.csManager->getProcessArray<uint8_t>(
                    oneProcessVariable->getName())->accessChannel(0);
            ...
            if (valueArray.size() == 1) {
                ...
                // Check value on controlsystemmanager side
                vector<uint8_t> csValueArray = pvSet.csManager->getProcessArray<uint8_t>(
                        oneProcessVariable->getName())->accessChannel(0);
                BOOST_CHECK(csValueArray.size() == 1);
                BOOST_CHECK(csValueArray.at(0) == 100);
            } else {
            ...
            vector<int16_t> valueArray = pvSet.csManager->getProcessArray<int16_t>(
                    oneProcessVariable->getName())->accessChannel(0);
            ...
            if (valueArray.size() == 1) {
                ...
                // Check value on controlsystemmanager side
                vector<int16_t> csValueArray = pvSet.csManager->getProcessArray<int16_t>(
                        oneProcessVariable->getName())->accessChannel(0);
                BOOST_CHECK(csValueArray.size() == 1);
                BOOST_CHECK(csValueArray.at(0) == 100);
            } else {
            ...
            vector<uint16_t> valueArray = pvSet.csManager->getProcessArray<uint16_t>(
                    oneProcessVariable->getName())->accessChannel(0);
            ...
            if (valueArray.size() == 1) {
                ...
                // Check value on controlsystemmanager side
                vector<uint16_t> csValueArray = pvSet.csManager->getProcessArray<uint16_t>(
                        oneProcessVariable->getName())->accessChannel(0);
                BOOST_CHECK(csValueArray.size() == 1);
                BOOST_CHECK(csValueArray.at(0) == 100);
            } else {
            ...
            vector<int32_t> valueArray = pvSet.csManager->getProcessArray<int32_t>(
                    oneProcessVariable->getName())->accessChannel(0);
            ...
            if (valueArray.size() == 1) {
                ...
                // Check value on controlsystemmanager side
                vector<int32_t> csValueArray = pvSet.csManager->getProcessArray<int32_t>(
                        oneProcessVariable->getName())->accessChannel(0);
                BOOST_CHECK(csValueArray.size() == 1);
                BOOST_CHECK(csValueArray.at(0) == 100);
            } else {
            ...
            if (valueArray.size() == 1) {
                ...
                // Check value on controlsystemmanager side
                vector<float> csValueArray = pvSet.csManager->getProcessArray<float>(
                        oneProcessVariable->getName())->accessChannel(0);
                BOOST_CHECK(csValueArray.size() == 1);
                BOOST_CHECK(csValueArray.at(0) == 100);
            } else {
            ...
            vector<double> valueArray = pvSet.csManager->getProcessArray<double>(
                    oneProcessVariable->getName())->accessChannel(0);
            ...
            if (valueArray.size() == 1) {
                ...
                // Check value on controlsystemmanager side
                vector<double> csValueArray = pvSet.csManager->getProcessArray<double>(
                        oneProcessVariable->getName())->accessChannel(0);
                BOOST_CHECK(csValueArray.size() == 1);
                BOOST_CHECK(csValueArray.at(0) == 100);
            } else {
        ...
        } else if (valueType == "Unsupported type") {
            BOOST_CHECK(valueType == "Unsupported type");
            cerr << "Warning: Unspported Type!" << endl;
        } else
            BOOST_CHECK(false);
...
void ProcessVariableTest::testClientSide() {
    ...
    for (ProcessVariable::SharedPtr oneProcessVariable : pvSet.csManager->getAllProcessVariables()) {
        varList.push_back(
                new ua_processvariable(serverSet->mappedServer, serverSet->baseNodeId, oneProcessVariable->getName(),
                                       pvSet.csManager));
    }
        ...
        for (size_t j = 0; j < bResp.results[i].referencesSize; ++j) {
            ...
            if (ref->nodeId.nodeId.identifierType == UA_NODEIDTYPE_NUMERIC) {
                ...
                for (size_t m = 0; m < bResp2.resultsSize; ++m) {
                    for (size_t k = 0; k < bResp2.results[m].referencesSize; ++k) {
                        ...
                        if (refe->nodeId.nodeId.identifierType == UA_NODEIDTYPE_NUMERIC ||
                            ...
                            string browseNameFound2(reinterpret_cast<char const *>(refe->browseName.name.data),
                                                    lenBrowseName2);
                ...
                if (!UA_NodeId_isNull(&valueNodeId)) {
                    ...
                    UA_StatusCode retvalValueDatatype = UA_Client_readValueAttribute(client, typeNodeId,
                                                                                     datatypeToCheck);
                    ...
                    UA_StatusCode retvalValueEu = UA_Client_readValueAttribute(client, engineeringUnitNodeId,
                                                                               euToCheck);
                    ...
                    UA_StatusCode retvalValueDesc = UA_Client_readValueAttribute(client, descriptionNodeId,
                                                                                 descToCheck);
                    ...
                    if (retvalValue == UA_STATUSCODE_GOOD) {
                        UA_StatusCode retvalDatatype = UA_Client_readDataTypeAttribute(client, valueNodeId,
                                                                                       &datatypeId);
                    ...
                    if (retvalValue == UA_STATUSCODE_GOOD) {
                        ...
                        UA_StatusCode retvalNewEU = UA_Client_writeValueAttribute(client, engineeringUnitNodeId,
                                                                                  euToCheck);
                        ...
                        UA_StatusCode retvalNewDesc = UA_Client_writeValueAttribute(client, descriptionNodeId,
                                                                                    descToCheck);
                        ...
                        if (valueToCheck->arrayLength < 1) {
                            switch (datatypeId.identifier.numeric - 1) {
                                case UA_TYPES_SBYTE: {
                                    // Check Datatype
                                    UASTRING_TO_CPPSTRING(((UA_String) *((UA_String *) datatypeToCheck->data)),
                                                          datatype);
                                    ...
                                    UA_StatusCode retvalNewVar = UA_Client_writeValueAttribute(client, valueNodeId,
                                                                                               valueToCheck);
                            ...
                            */
                            switch (datatypeId.identifier.numeric - 1) {
                                case UA_TYPES_SBYTE: {
                                    ...
                                    UA_Variant_init(valueToCheck);
                                    UA_Variant_setArrayCopy(valueToCheck, newValue, arrayLength,
                                ...
                                case UA_TYPES_DOUBLE: {
                                    ...
                                    if (retvalNewVar == UA_STATUSCODE_GOOD) {
                                        ...
                                        for (uint32_t i = 0; i < arrayLength; i++) {
                                            BOOST_CHECK(
                                                    ((int32_t) (value[i] * 100)) == ((int32_t) (newValue[i] * 100)));
                                        }
                                    } else {
                                        BOOST_CHECK(false);
                                    }
